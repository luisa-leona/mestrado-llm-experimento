...
int main(int argc, char ** argv) {
    // Extract Optane arguments manually to avoid common_params_parse error
    std::string         optane_placement;
    std::string         optane_url;
    std::vector<char *> new_argv;
    for (int i = 0; i < argc; ++i) {
        std::string arg = argv[i];
        if (arg == "--optane-placement") {
            if (i + 1 < argc) {
                optane_placement = argv[++i];
            } else {
                fprintf(stderr, "error: --optane-placement requires an argument\n");
                return 1;
            }
        } else if (arg == "--optane-url" || arg == "--pmem-path") {
            if (i + 1 < argc) {
                optane_url = argv[++i];
            } else {
                fprintf(stderr, "error: --optane-url requires an argument\n");
                return 1;
            }
        } else {
    ...
    // Update argc/argv for common_params_parse
    int     new_argc     = (int) new_argv.size();
    char ** new_argv_ptr = new_argv.data();

    common_params params;
    g_params = &params;
    if (!common_params_parse(new_argc, new_argv_ptr, params, LLAMA_EXAMPLE_MAIN, print_usage)) {
        return 1;
    }
